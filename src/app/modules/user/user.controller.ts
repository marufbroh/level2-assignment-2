...
const addProductToOrder = async (req: Request, res: Response) => {
  try {
    ...
    await userServices.addProductToOrder(userId, zodParsedData)
    res.status(200).json({
      success: true,
...
const getTotalPriceOfOrdersFromUser = async (req: Request, res: Response) => {
  try {
    const userId = Number(req.params.userId)
    const result = await userServices.getTotalPriceOfOrdersFromUser(userId)
    res.status(200).json({
      success: true,
